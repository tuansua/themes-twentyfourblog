...
<div id="post-<?php the_ID(); ?>" <?php post_class( $classes ); ?>>
				...
				<div class="column one post-header">
					...
					<div class="title_wrapper">
						...
						<?php 
							...
							//CUSTOM POSTMETA
							$custom_post_meta = get_post_meta(get_the_ID());
							$cus_time = get_the_time('H:i',get_the_ID());
							$cus_date = get_the_date('d/m/Y',get_the_ID());
							$cus_cate = get_the_category(get_the_ID());
						...
						<div class="custom-date-time-meta">
							<?php
								echo '<span class="cus-time">'.$cus_time.'</span><span class="cus-date">'.$cus_date.'</span>';
								echo the_category();
							?>
						</div>
	...
	<div class="post-wrapper-content">
		...
		<?php
			//Link 3 bài viết liên quan
			$aCategories1 = wp_get_post_categories( get_the_ID() );

			$args = array(
						'category__in'			=> $aCategories1,
		    ...
		    if ( $myposts ) {
		    	echo '<div class="tin-lien-quan custom-tinlienquan-1 ">
							<ul class="ul-first">';
		        foreach ( $myposts as $post ) :
		            setup_postdata( $post );

		        	echo '<li class="custom-blog">
							<h4><a href="'.get_post_permalink($post->ID).'" title="'.$post->post_title.'">'.$post->post_title.'</a></h4>
						</li>					
		        	';
							...
							echo '<div class="section-related-ul '. $related_cols .'">';
								...
								while ( $query_related_posts->have_posts() ){
									...
									echo '<div class="column post-related '. implode( ' ', get_post_class( $related_class ) ).'">';	
										...
										echo '</div>';
										echo '<div class="custom-date-time-meta">
												
												<span class="cus-time">'.$cus_time.'</span><span class="cus-date">'.$cus_date.'</span>
